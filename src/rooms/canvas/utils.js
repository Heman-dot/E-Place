...
export const toggleTooltip = async (state = false) => {
    ...
    if (state) {
        const posX = target.x;
        const posY = target.y;
        const slug = window.location.pathname.slice(1) || "epi-place";

        try {
            const res = await fetch(
                `/api/rooms/${slug}/canvas/pixels?posX=${posX}&posY=${posY}`,
            ...
            document.getElementById("tooltip-info-login").textContent =
                info.login ?? info.placedByUid;
            document.getElementById("tooltip-info-quote").textContent =
                info.quote ?? "";
            document.getElementById("tooltip-info-avatar").src =
                info.avatarURL ?? "/default-avatar.png";
        ...
        const config = {
            ...
            url: `${import.meta.env.VITE_API_URL}/api/rooms/${slug}/canvas/pixels`,
            headers: {
        ...
        try {
            ...
            console.log("Pixel POST response:", response.data);
        } catch (error) {
